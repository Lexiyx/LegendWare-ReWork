...
{
	if (!g_cfg.ragebot.defensive_doubletap || !g_ctx.send_packet)
		return;
	...
	if (g_ctx.globals.m_Peek.m_bIsPeeking) // проверка на пик, это тока обявление -- сам код из аимбота надо взять из itzlaith_lw.
	{
...
void misc::lagexploit(CUserCmd* m_pcmd)
{
	...
	if (!g_ctx.local()->m_vecVelocity().Length2D() == 0.f)
	{
	...
	if (did_peek && g_cfg.misc.break_lc)
	{
...
void misc::double_tap(CUserCmd* m_pcmd)
{
	...
	//auto recharge_time = maxshift * m_globals()->m_intervalpertick;

	auto weapon = g_ctx.local()->m_hActiveWeapon();


	if (!(m_pcmd->m_buttons & IN_ATTACK) && (g_ctx.globals.tocharge < maxshift || g_ctx.globals.tocharge <= 0) && g_ctx.globals.fixed_tickbase - lastdoubletaptime > recharge_time)
	{
		...
		g_ctx.globals.tochargeamount = maxshift;// shifting happens here
